...
while ($p = mysqli_fetch_array($peminjaman)) {
  ...
  $pengembalian = mysqli_query($koneksi, "SELECT COUNT(*) as count FROM t_pengembalian WHERE id_pmnjmn = $id_pmnjmn");
  $pe = mysqli_fetch_assoc($pengembalian);

  // Apply red color if the book is not returned
  $rowColor = ($pe['count'] == 0) ? 'style="background-color: #ffcccc;"' : '';
?>
